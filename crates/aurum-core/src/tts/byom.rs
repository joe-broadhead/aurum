...
use super::adapter::{
    list_adapters, lookup_adapter, preflight_manifest, ModelPackManifest, TrustMode,
...
pub fn propose_add_local(
    ...
    let _adapter = lookup_adapter(adapter_id)?;
    ...
    let mut manifest = if manifest_path.is_file() {
        ...
        warnings.push(format!(
            "no {MANIFEST_FILENAME} yet — generating a skeleton; fill digests before verified trust"
        ));
        ModelPackManifest {
...
pub fn builtin_kitten_manifest_json() -> Result<String> {
    ...
    serde_json::to_string_pretty(&m).map_err(|e| {
        crate::error::TranscriptionError::internal(format!("manifest json: {e}"))
    })
}
