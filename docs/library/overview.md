...
|---------|------------------|
| `aurum-core` | `aurum-core = "0.0.5"` (default features include TTS; use `default-features = false` for STT-only) |
| `aurum-stt` | `cargo install aurum-stt` → runs `aurum` |
| `aurum-ffi` | `aurum-ffi = "0.0.5"` or build from source (`libaurum_ffi` + `aurum.h`) |
