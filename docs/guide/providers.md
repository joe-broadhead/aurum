...
| App attribution | Every OpenRouter request sends [OpenRouter app headers](https://openrouter.ai/docs/app-attribution): `HTTP-Referer` = `https://github.com/joe-broadhead/aurum`, `X-OpenRouter-Title` = `Aurum`, `X-OpenRouter-Categories` = `audio-gen,cli-agent`. Other providers never receive these headers. |
